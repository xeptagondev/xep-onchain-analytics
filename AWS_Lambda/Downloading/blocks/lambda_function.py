...
def handler(event = None, context= None):
    ...
    if update_information["last_data_uploaded"] == str(today):
        print("No downloads required since lastest data is already uploaded")
        return
...
def download_from_blockchair(folders, blockchair_key = blockchair_key, number_of_files = num_files, data_type = data_type, s3_client = s3_client):
    ...
    for _ in range(number_of_files):
        ...
        try:
            ...
            print(e)
            break
        print("Downloaded " + folder) # zip folder is downloaded .tsv.gz
