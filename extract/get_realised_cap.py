...
def get_realised_cap(start_date, end_date, conn, config):
    ...
    str2 = "...),time(.."
    str3 = ")&export=csv&key=" + config['APIkey']['blockchair']
    ...
    for i in range(delta.days + 1):
        ...
        df = pd.read_csv(url)
        df.rename(columns = {'sum(value)': 'value_btc', 'sum(value_usd)': 'value_usd'}, inplace = True)

        realised_cap_usd = df['value_usd'].sum()
        realised_cap_btc = df['value_btc'].sum() / 100000000
        realised_cap_df = pd.DataFrame({'Date':[day], 'Realised_Cap_USD':[realised_cap_usd], 'Realised_Cap_BTC_':[realised_cap_btc]})
        # Inserts the newly computed realised cap into duckdb
        conn.execute("""INSERT INTO realised_cap SELECT * FROM realised_cap_df""")
